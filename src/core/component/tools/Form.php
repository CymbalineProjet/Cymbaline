...
use core\component\validator\FormValidator as Validator;
/**
 * Description of Form
 *
 * @author Thibault
...
class Form extends Validator {
	...
	public function add($name, $type = "text", array $attributs = NULL, array $label = NULL, array $choices = NULL) {
        ...
        switch ($type) {
			...
			case 'hidden':
                $this->createTypeHidden($name,$attributs,$label);
            break;
	...
	public function createTypeHidden($name,array $attributs,array $label) {
        $attr = "";
        foreach ($attributs as $a => $v) {
                $attr .= "$a='$v' ";
        }
        $this->fields["$name"] = "<input type='hidden' name='".$this->name."[$name]' $attr />";
